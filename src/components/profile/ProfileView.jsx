...
const ProfileView = ({ profile, onEdit }) => {
  ...
  return (
    <div className="p-4 sm:max-w-4xl bg-white rounded-[8px]">
      <div className="sm:flex gap-[60px] justify-start items-center p-10 grid grid-cols-2 divide-x-2">
        ...
        <div className="pl-[60px]">
          <div className="flex gap-2 items-center text-lg">
            <FaRegUser />
            <p>{capitalizeFirstLetter(userInfo?.name)}</p> {/* Capitalize name */}
          </div>

          <div className="flex gap-2 items-center text-lg">
            <MdOutlineAlternateEmail />
            <p>{userInfo?.email || 'No email provided'}</p> {/* Fallback for email */}
          </div>
